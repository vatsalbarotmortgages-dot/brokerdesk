...
// Stages that should NOT receive partner status emails
const EXCLUDED_STAGES = ['Funded', 'Old Files — Funded', 'Check In', 'Info Only / No Deal', 'Cancelled']
...
export default async function handler(req, res) {
  ...
  deals.forEach(d => {
    ...
    if (!key) return
    if (!partnerMap[key]) {
      partnerMap[key] = { name: d.ref_name, email: d.ref_email, deals: [], latestEntry: d.entry_date }
    }
    partnerMap[key].deals.push(d)
    if (d.entry_date > partnerMap[key].latestEntry) partnerMap[key].latestEntry = d.entry_date
  })
  ...
  for (const [key, partner] of Object.entries(partnerMap)) {
    ...
    // Check no_partner_email flag
    const hasNoEmailFlag = partner.deals.some(d => d.no_partner_email)
    if (hasNoEmailFlag) continue

    // Active deals — exclude certain stages
    const activeDeals = partner.deals.filter(d => !EXCLUDED_STAGES.includes(d.stage))

    // Bi-weekly status — only for active deals
    ...
    // Coffee invite — only if no referrals in 60 days
