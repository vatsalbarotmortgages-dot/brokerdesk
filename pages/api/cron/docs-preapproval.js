...
// Only these stages get docs/pre-approval follow-ups
// Check In and Old Files — Funded are intentionally excluded
const ELIGIBLE_STAGES = ['Documents Received', 'Pre-Approval']
    ...
    for (const deal of deals) {
      ...
      for (const [targetDays, emailNum] of [[7, 1], [21, 2]]) {
        ...
        const { data: log } = await supabase
          ...
          .eq('deal_id', deal.id).eq('email_type', emailType).limit(1)
        ...
        await supabase.from('email_log').insert({
          deal_id: deal.id, email_type: emailType, recipient: deal.b1_email
        })
        sent++
      }
    }
